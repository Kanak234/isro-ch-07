...
def flatten(t, flux, window_frac=0.05, polyorder=2, protect_sigma=3.0):
    """
    Detrend using Savitzky-Golay. Dips (potential transits) are masked out
    when estimating the trend so they survive the flattening.
    """
    n = len(flux)
    ...
    # first pass trend
    trend = savgol_filter(flux, win, polyorder)
    resid = flux - trend
    ...
    trend2 = np.interp(t, t[keep], savgol_filter(flux[keep], 
